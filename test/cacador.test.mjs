...
test('demo Bruno: João + Maria com linha canônica', () => {
  ...
  assert.match(linhaLead(leads[0], { agora }), /João Silva - 11 9xxxx - FortMyers BR_SC - há 12min - Novo/);
  assert.match(linhaLead(leads[1], { agora }), /Maria - 47 9xxxx - Amanay Itapoá - há 34min/);
...
test('1 toque Bom → toast CPL BOM + totais campanha', () => {
  ...
  assert.equal(r.totaisCampanha.campanha, 'FortMyers BR_SC');
