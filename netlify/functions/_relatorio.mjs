// Construtor do relatório consolidado (arquivo "_" = não vira função).
// Gera o resumo do WhatsApp e o HTML do e-mail (preto+dourado Katzer) a partir do estado do dia.
import { TAREFAS, previstoMin, min2hm, pontualidade } from './_rotina.mjs';
...
/** Resumo curto pro WhatsApp do CEO. */
export function resumoWhats(estado, now) {
  const P = pontualidade(estado, now.min);
  const modoIco = estado.modo === 'katzer' ? '🏢 Katzer' : '🏠 Casa';
  ...
  const atrasadas = P.linhas.filter((l) => l.feito && l.difMin > 0).length; // feitas, mas fora da hora
  const pendentes = P.linhas.filter((l) => !l.feito).length;
  const destaques = P.linhas
    .filter((l) => l.feito && l.difMin > 0) // só atrasos REAIS (feitos); pendentes vão no contador
    .map((l) => `🔴 ${l.nome} −${l.difMin}′`).join(' · ');
  const obs = (estado.obs || []).map((o) => `🔔 ${o.quem}: ${o.nome}${o.duracao ? ` (${o.duracao}min)` : ''}`).join('\n');
  const linhas = [
    `📊 *Rotina do Michel* — ${dataExtenso(estado.data, now.dow)} · ${modoIco}`,
...
/** E-mail HTML (inline styles p/ compatibilidade) — preto + dourado Katzer. */
export function emailHTML(estado, now) {
  const P = pontualidade(estado, now.min);
  const modo = estado.modo === 'katzer' ? '🏢 Katzer' : '🏠 Casa';
  ...
  const obsHtml = (estado.obs || []).length ? `
    <div style="margin:16px 22px 0;border:1px solid #8a6c2e;border-radius:10px;background:rgba(201,162,74,.05);padding:12px 14px">
      <div style="color:#c9a24a;font-size:12px;letter-spacing:.1em;text-transform:uppercase;font-weight:700">🔔 Obs — você/Carol pediram</div>
    ...
    <div style="max-width:600px;margin:0 auto;background:#0e0e11;border:1px solid #2a2732;border-radius:14px;overflow:hidden">
      ...
      <div style="padding:22px;display:flex;align-items:center;gap:18px">
        ...
        <div><div style="color:#9a9283;font-size:11px;letter-spacing:.14em;text-transform:uppercase">Meta do dia</div>
          <div style="color:#ece5d6;font-size:14px;margin-top:4px">Saldo ${P.saldoMin > 0 ? '−' + P.saldoMin : '+' + (-P.saldoMin)}′ · 100% − ${Math.max(0, P.saldoMin)} min de atraso</div></div>
      </div>
      ...
      <div style="padding:18px 22px;color:#9a9283;font-size:12px;text-align:center;border-top:1px solid #2a2732;margin-top:16px">
        Enviado ${estado.modo === 'katzer' ? '14:30 (Katzer)' : '13:30 (Casa)'} · seg–sáb · o % é do gestor.
      </div>
    </div></body></html>`;
}
